...
export interface aoaInfo {
    ...
    address: string,
    citystatezip: string,
    allPropertyListedAbove: boolean,
    propertyListedBelow: boolean,
...
const mapToPdfFieldNames = (pdfForm: PDFForm, newAoaData: aoaInfo) => {
    ...
    if (newAoaData.allPropertyListedAbove) propertiesListedAboveCheck.check();
    if (newAoaData.propertyListedBelow) propertiesListedBelowCheck.check();
